...
>
> **更新**：2026-09-09（**P2.8 Batch 3**，见 `docs/history/113-p28-batch3-transactional-install.md`；权威逐条基线 `demo2/files2/97-launcher-1.0-final-audit.md`）。基线：743 tests 全绿 / zero warnings / topology 17 crates + 9 apps / S0=S1=S2=0 / Release Gate G01~G14 PASS / 1.0 manifest=released（签名待补）/ hotkey P50 307µs·P95 19.7ms / 10k UI soak PASS。
...
P28-Batch3    事务化 Install（stage→activate +          ✅ Batch 3
              逆序回滚）—— 113 号
P28-Batch4-6  Signature+Trust/Repository/UI/收口        ⏳ 未开始
...
```bash
cargo test --workspace          # 行为回归闸门（当前 743 全绿）
